...
/* Start / Stop Virtualcam Action

Toggles the Virtual Camera feature.

State:
- Enabled if OBS virtual cam is active.
- Disabled if OBS virtual cam is inactive or OBS is closed.
...
- Request:
	1. Enable / Disable virtual camera according to target state.
	2. If success, become "Enabled"/"Disabled" according to target state, otherwise become "Disabled", show error and then break.
- obs.frontend.source.event.virtualcam:
	1. Refresh all virtualcam actions.
- Refresh:
	1. Check if OBS is connected, otherwise show error and then break.
	2. Retrieve current virtualcam state.
	3. If success, become "Enabled"/"Disabled" according to state, otherwise become "Disabled", show error and then break.

States:
- EActionOn: OBS is connected, OBS virtual cam is active.
- EActionOff: OBS is disconnected or OBS virtual cam is inactive .
*/

class VirtualCamAction extends IAction {
	constructor(controller) {
		super(controller, "com.elgato.obsstudio.virtualcam");

		this._timer = null;
		this._api = this.obs.frontend_virtualcam;
		...
		{ // Listeners
			...
			this._api.addEventListener("status", (event) => {
				this._on_virtualcam_status(event);
			});
		} catch(e){
			this.obs.log(e);
		}
