...
/**
 * The currently selected sub ID
 */

var currentSelectedSubID;
...
$SD.on('connected', (jsn) => {
	...
	if (actionType) {
		...
		const subLabel = document.querySelector( "#select_combo_sub_label" );
		if (actionType == "tv.twitch.studio.audio") {
			label.innerText = 'Audio Source'.lox();
		} else if (actionType == "tv.twitch.studio.layout") {
			label.innerText = 'Layout'.lox();
		} else if (actionType == "tv.twitch.studio.layer") {
			label.innerText = 'Layout'.lox();
			subLabel.innerText = 'Layer'.lox();
		}
		
		const mainCombo = document.querySelector( "#select_combo" );
		const subCombo = document.querySelector( "#select_sub_combo" );
		if (actionType == "tv.twitch.studio.layer") {
			mainCombo.style.visibility = "visible";
			subCombo.style.visibility = "visible";
		} else if (actionType == "tv.twitch.studio.audio" || actionType == "tv.twitch.studio.layout" ) {
			mainCombo.style.visibility = "visible";
			subCombo.style.visibility = "hidden";
			subCombo.style.display = "none";
		} else {
    ...
    if (settings) {
		...
		if (settings.hasOwnProperty('currentSelectedSubID')) {
			currentSelectedSubID = settings['currentSelectedSubID'];
		} else {
			currentSelectedSubID = "";
		}
...
$SD.on('sendToPropertyInspector', jsn => {
	...
	if( jsn.payload && jsn.payload.hasOwnProperty('layers')) {
		const layers = jsn.payload['layers'];
		
		const el = document.querySelector( '#currentSelectedID' );
		el.innerHTML = ""
		
		const subEl = document.querySelector( '#currentSelectedSubID' );
		subEl.innerHTML = ""
		
		if (layers !== undefined) {
			const adjustLayersSelect = ( layersArr, selectedID ) => {
				return layersArr.map( layer => `<option value="${ layer.id }" ${ layer.id == selectedID ? 'selected' : '' }> ${ layer.name }</option >` ).join( ' ' );
			};
			
			el.innerHTML = adjustLayersSelect( layers, currentSelectedID );

			const initialLayer = layers.find( l => l.id == currentSelectedID );
			if ((initialLayer !== undefined) && (initialLayer.layers !== undefined) && (initialLayer.layers.length > 0)) {
				subEl.innerHTML = adjustLayersSelect( initialLayer.layers, currentSelectedSubID );
			} else {
				subEl.innerHTML = ""
			}
			...
			el.onchange = () => {
				...
				currentSelectedSubID = ""
				if (actionType && actionType == "tv.twitch.studio.layer") {
					const newSelectedLayer = layers.find( l => l.id == currentSelectedID );
					if ((newSelectedLayer !== undefined) && (newSelectedLayer.layers !== undefined) && (newSelectedLayer.layers.length > 0) ) {
						currentSelectedSubID = newSelectedLayer.layers[0].id
						subEl.innerHTML = adjustLayersSelect( newSelectedLayer.layers, currentSelectedSubID );
					} else {
						subEl.innerHTML = ""
					}
				}
				...
				settings["currentSelectedSubID"] = currentSelectedSubID;
				$SD.api.setSettings($SD.uuid, settings);
			};
		
			subEl.onchange = () => {
				currentSelectedID = el.value;
				currentSelectedSubID = subEl.value;
