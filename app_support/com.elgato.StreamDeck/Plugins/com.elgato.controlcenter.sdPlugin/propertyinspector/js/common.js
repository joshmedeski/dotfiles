...
// force a specific language for localization
// let MFORCELOCALIZATIONLANGUAGE = 'de';
...
const loadLocalization = (lang, pathPrefix, cb) => {
    if(typeof MFORCELOCALIZATIONLANGUAGE !== 'undefined') lang = MFORCELOCALIZATIONLANGUAGE;
    Utils.readJson(`${pathPrefix}${lang}.json`, function(jsn) {
        ...
        debugLog({manifest, $localizedStrings});
        if(cb && typeof cb === 'function') cb($localizedStrings);
    });
};
...
/* quick compare of two objects */
// console.log(Object.entries(a).sort().toString()===Object.entries(b).sort().toString()) //true  ??

Utils.minmax = function(v = 0, min = 0, max = 100) {
    return Math.min(max, Math.max(min, v));
};

Utils.cycle = (idx, min, max) => (idx > max ? min : idx < min ? max : idx);
...
Utils.setDebugOutput = (debug, LOG_PREFIX, tagClr) => {
    if(LOG_PREFIX || tagClr) {
        const prefix = LOG_PREFIX? `[${LOG_PREFIX}] ` : '';
        const clr = tagClr? tagClr : '';
        console.log("debugLog", {prefix, clr, LOG_PREFIX});
        return debug === true ? console.log.bind(window.console, `%c${prefix} %s`, `color:${clr};`) : function() {};
    } else {
...
Utils.shuffleArray = function(inArr) {
...
Utils.uuidv4 = function() {
    const dec2hex = Array.from(Array(16), (e, i) => i.toString(16));
    let uuid = '';
    for(let i = 1;i <= 36;i++) {
...
Utils.btoaUTF16 = function(sString) {
...
Utils.atobUTF16 = function(sBase64) {
...
Utils.encodeFileName = function(inFileName) {
    return `${Date.now().toString(36)}|${Utils.utf8_to_b64(inFileName)}`;
};

Utils.encodeFileName2 = function(inFileName) {
    return Utils.utf8_to_b64(`${Date.now().toString(36)}|${inFileName}`);
};
...
Utils.fixName = (d, dName = '', emptyNameString = '') => {
    let i = 1;
    const base = dName.length ? dName : emptyNameString;
    while(d[dName]) {
        dName = `${base} (${i})`;
        i++;
    }
    return dName.length ? dName : emptyNameString;
    return dName;
};

Utils.fixNameWithCallback = (d, dName = '', emptyNameString = '', cb) => {
    let i = 1;
    const base = dName.length ? dName : emptyNameString;
    while(d[dName]) {
        dName = cb(base, i); //`${base} (${i})`;
        i++;
    }
    return dName.length ? dName : emptyNameString;
    return dName;
...
Utils.getUrlParameters = function() {
    ...
    if(search) {
        fields = JSON.parse(`{${decodeURI(search)
            .replace(/"/g, '\\"')
            .replace(/&/g, '","')
            .replace(/=/g, '":"')}
        }`);
...
Utils.isTemperature = function(h, s) {
    let bool1 = Utils.IsColorDisplayableAsTemperature(h, s);
    let bool2 = (s < .6 && h < 30);
    let bool3 = (s < .1 && h > 240);
    return bool1 || bool2 || bool3;
};

Utils.IsColorDisplayableAsTemperature = function(hue, saturation) {
    let isDisplayable = false;
    if(hue >= 38 && hue <= 42) {
        isDisplayable = true;
    }
    else if(hue >= 198 && hue <= 202) {
        isDisplayable = true;
    }
        ...
        else if(hue >= 198 && hue <= 212) {
            isDisplayable = true;
        }
        else if(hue <= 1 || hue > 359) {
            isDisplayable = true;
        }
    }
...
Utils.readJson = function(file, callback, async = true) {
...
Utils.readFile = (fileName, props = {}) => {
    if(!props || !props.hasOwnProperty('responseType')) {
        const suffix = fileName.split(".").pop();
        switch(suffix) {
            case 'json': {
                props.responseType = 'json';
                props.overrideMimeType = 'application/json';
            }
        }
    }
        ...
        request.onload = (e, f) => {
            ...
            if(isBlob) {
                ...
                if(jsn) {
                    ...
                    console.warn('[Error]::ReadFile.reply was not JSON', request);
                    resolve({});  // reject('No JSON response!');
                }
...
Utils.getProperty = function(obj, dotSeparatedKeys, defaultValue) {
    ...
    if(arguments.length > 1 && typeof dotSeparatedKeys !== 'string') return defaultValue;
    if(typeof obj == 'undefined' || typeof dotSeparatedKeys !== 'string') return defaultValue;
    ...
    obj = pathArr.reduce((o, key) => (o && o[key] ? o[key] : undefined), obj);
    // obj = pathArr.reduce((o, key) => (o && o[key] !== 'undefined' ? o[key] : undefined), obj);
    // obj = pathArr.reduce((o, key) => (o && o.hasOwnProperty(key) ? o[key] : defaultValue), obj);
    // }
...
Utils.setProp2 = function(jsonObj, path, value) {
    const [head, ...rest] = path.split('.');

    !rest.length
        ? jsonObj[head] = value
        : this.setProp2(jsonObj[head], value, rest);
};
...
Utils.s2c = function(svg, inCanvas, inContext, w, h, cb, x = 0, y = 0) {
    var img = new Image();
    img.src = svg;
    img.onload = function() {
...
Utils.utoa = function(data) {
    return btoa(unescape(encodeURIComponent(data)));
};

Utils.toBase64 = function(svg, containsUnicode = false) {
    // return `data:image/svg+xml;base64,${utoa(svg)}`;
    return containsUnicode ? `data:image/svg+xml;base64,${Utils.utoa(svg)}` : `data:image/svg+xml;base64,${btoa(svg)}`;
};

Utils.drawStringToCanvas = (num, size = 400, inCanvas) => {
    ...
    tempCtx.fillText(`${num}`, canvas.width / 2, canvas.height / 2);
};
...
Utils.updateStyle = function(styleId, styles) {
    const tempID = styleId || Utils.randomString(8);
    const foundStyle = document.querySelector(`style[id="${tempID}"]`);
    if(!foundStyle) {
        Utils.injectStyle(styles, styleId);
    } else {
        foundStyle.textContent = styles;
    }
    return foundStyle;
};
    ...
    function init() {
        ...
        function connect(args) {
            ...
            websocket.onopen = function() {
                ...
                const [version,major,minor, build] = $SD.applicationInfo.application.version.split(".").map(e => parseInt(e, 10));
                // inApplicationInfo.application.semver = {version, major, minor, build};
                inApplicationInfo.application.features = {
                    hasDialPress: version == 6 && major < 4
                }
            ...
            websocket.onclose = function(evt) {
                ...
                console.warn('[STREAMDECK]***** WEBOCKET CLOSED **** reason:', reason, evt);
            };
...
const SDApi = {
    ...
    common: {
        ...
        openUrl: function() {
            /**
             * for openUrl we don't need a context, so we allow both
             * openUrl(unneededContext, 'message')
             * and
             * openUrl('message')
             */

            let urlToOpen = arguments.length > 1 ? arguments[1] : arguments[0];

            SDApi.send(null, 'openUrl', {
                payload: {
                    url: urlToOpen
                }
            });
...
SDApi.registerIconEditor = SDApi.registerPlugin;
window.SDApi = SDApi;
