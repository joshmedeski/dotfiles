...
const ControlCenterKey = function(jsn, options, debugColor) {
    ...
    this.allowedDeviceTypes = (options && options.allowedDeviceTypes) || DEVICETYPES.KEYLIGHTS;
    ...
    this.onDialDefaultAction = function(jsn) {
        const device = this.settings && this.findDeviceById(this.settings['deviceID']);
        if(this.onDialAction) {
            this.onDialAction(device, jsn);
        };
        if(device?.lights) {
            device.lights.on = !device.lights.on;
    ...
    if(this.sd.applicationInfo.application.features.hasDialPress === true) {
        this.listeners.push(
            this.sd.on(`${this.action}.dialPress`, (jsonObj) => {
                if(jsonObj['context'] === this.context) {
                    // self.onDialPress(jsonObj);
                    if(jsonObj.payload.pressed === false) {
                        self.onDialUp(jsonObj);
                    } else {
                        self.onDialDown(jsonObj);
                    }
                }
            })
        );
        ...
        this.listeners.push(
            this.sd.on(`${this.action}.dialUp`, (jsonObj) => {
                if(jsonObj['context'] === this.context) {
                    jsonObj.payload.pressed = false;
                    self.onDialUp(jsonObj);
                }
            })
        );
        ...
        this.listeners.push(
            this.sd.on(`${this.action}.dialDown`, (jsonObj) => {
                if(jsonObj['context'] === this.context) {
                    jsonObj.payload.pressed = true;
                    self.onDialDown(jsonObj);
                }
            })
        );
